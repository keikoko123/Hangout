...
class _DashboardPageState extends State<DashboardPage>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      print("Dashboard initState: Forcing fresh data fetch");
      _fetchUserDirectly();
    });
  }
  ...
  void didChangeDependencies() {
    ...
    // Always fetch fresh data when page dependencies change
    print("Dashboard didChangeDependencies: Forcing fresh data fetch");
    _fetchUserDirectly();
  }
  ...
  Future<void> _fetchUserDirectly() async {
    ...
    try {
      ...
      print(
          "Dashboard: Fetching FRESH user data from API: $_baseUrl/users/$userId");
      ...
      if (response.statusCode == 200) {
        ...
        print("Dashboard: Successfully fetched fresh user data");
        print("Raw response data: ${response.body}");

        // Clear any existing user cubit data and force a refresh
        // Get a new reference to avoid any cached data
        context.read<UserCubit>().clearUserData();

        // Update the UserCubit with fresh data
        final userCubit = context.read<UserCubit>();
        userCubit.fetchUserById(userId);
        ...
        print("Dashboard: Failed to fetch user by ID: ${response.statusCode}");
      ...
      print("Dashboard: Error fetching user directly: $e");

      // Fall back to UserCubit on error
      try {
        await context.read<UserCubit>().refreshCurrentUser();
      } catch (cubError) {
        print("Dashboard: Error refreshing user cubit data: $cubError");
      }
        ...
        actions: [
          IconButton(
            ...
            onPressed: _isRefreshing
                ? null
                : () {
                    // Manually refresh data when the user taps the refresh icon
                    _fetchUserDirectly();
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
